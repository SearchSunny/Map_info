...
public class ScrollLayout extends ViewGroup {
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
	{
		...
		Log.e("layout", "mCurScreen" + mCurScreen);
	...
	{ // getScrollX()返回的是向左滑动的总长度 screenWidth / 2 当滑动的距离大于一般的长度的时候就+1，向前或者向后滑动
	...
	public void snapToScreen(int whichScreen)
	{
		...
		if (getScrollX() != (whichScreen * getWidth()))
		{
			...
			LogPrint.Print("mCurScreen==="+mCurScreen);
			doScrollAction(mCurScreen);
	...
	public boolean onTouchEvent(MotionEvent event)
	{
		...
		switch (action)
		{
			...
			case MotionEvent.ACTION_UP:
				...
				int velocityX = (int) velocityTracker.getXVelocity();// 想右滑动手指为正
				Log.i("event_s", "velocityX" + velocityX);
				if (velocityX > SNAP_VELOCITY && mCurScreen > 0)
				{
					// Fling enough to move left
					snapToScreen(mCurScreen - 1);
				} else if (velocityX < -SNAP_VELOCITY && mCurScreen < getChildCount() - 1)// 想左滑动手指为正
				{
	...
	public boolean onInterceptTouchEvent(MotionEvent ev)
	{
		...
		{
			case MotionEvent.ACTION_DOWN:
				Log.i("event_s", "Intercept_ACTION_DOWN");
				mLastMotionX = x;
				mLastMotionY = y;
			...
			case MotionEvent.ACTION_MOVE:
				...
				Log.i("event_s", "Intercept_ACTION_MOVE");
			...
			case MotionEvent.ACTION_CANCEL:
			case MotionEvent.ACTION_UP:
				Log.i("event_sd", "Intercept_ACTION_UP");
				mTouchState = TOUCH_STATE_REST;
				break;
		}
